...
$sql2 = "SELECT e.empid, e.name, ISNULL((SELECT COUNT(l.leavedate) FROM leave l 
          WHERE l.empid = e.empid AND YEAR(l.leavedate) = YEAR(GETDATE()) AND l.action = 'A'), 0) AS leavecount, 
          (SELECT MAX(l.leavedate) FROM leave l 
          WHERE l.empid = e.empid AND l.leavedate < GETDATE() AND l.action = 'A') AS lastleavedate FROM emp e WITH (NOLOCK)
          WHERE e.emptype != 'TEST' AND e.flag = 'Y' AND e.empid NOT IN ('1108', '1070')
...
while ($row2 = sqlsrv_fetch_array($res2, SQLSRV_FETCH_ASSOC)) 
{
    ...
    $formattedLastleavedate = $lastleavedate ? DateTime::createFromFormat('Y-m-d', $lastleavedate)->format('jS M Y') : null;
    ...
    $fontsize = ($empid == $myempid) ? 'font-size:22px;' : '';
    ...
    $trdata .= "
        <tr class='dis' style='font-family:Poppins;padding:6px;'>
            ...
            <td align='center' style='$color $fontsize font-family:Poppins;padding:6px;'>$name</td>
            <td align='center' style='$color $fontsize font-family:Poppins;padding:6px;'>$leavecount</td>
            <td align='center' style='$color $fontsize font-family:Poppins;padding:6px;'>$formattedLastleavedate</td>
        </tr>";
...
<table style='margin-left:-15;' class="report" style='border-radius:10px;box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);'>
<tr>
