...
if ($_SERVER["REQUEST_METHOD"] == "POST") 
{
   ...
   $sql2 = "  INSERT INTO [MISGlobal].[dbo].[leave] (empid, leavedate, leavetype, compodate, reason) SELECT $empid , '$leavedate', '$leavetype', '$compodate', '$reason' WHERE NOT EXISTS ( SELECT 1 FROM [MISGlobal].[dbo].[leave] WHERE empid = $empid AND leavedate ='$leavedate' AND action = 'A')";
...
$profilePic='';
    ...
    $sql2 = "select DISTINCT empid, leavedate, compodate, reason, leavetype, time from leaverequest where pending = 'Y' order by time desc";
    $res2 = sqlsrv_query($conn, $sql2);
...
<button type="button" class="sub1" style="margin-left:980px;" onclick="rejectedLeaveRequests()">REJECTED LEAVE REQUESTS</button>

<div id="rejectedRequestsModal" style="display: none; position: fixed; top: 20%; left: 10%; width: 75%; background-color: white; border: 1px solid #ccc; box-shadow: 0 4px 8px rgba(0,0,0,0.2); padding: 20px; z-index: 1000;border-radius:10px;">
    <h3 style="text-align: center;">Rejected Leave Requests</h3>
    <div id="rejectedRequestsContent" style="max-height: 300px; overflow-y: auto;">
        <!-- Table will be populated here -->
    </div>
    <span class="close1-btn" onclick="closerejectedLeaveRequests()">×</span>
</div>
...
function rejectedLeaveRequests() 
{
    ...
    document.getElementById("rejectedRequestsModal").style.display = "block";
    document.getElementById("overlay").style.display = "block";

    fetch('rejectedleaves.php')
        .then(response => response.text())
        .then(data => 
		{
            document.getElementById("rejectedRequestsContent").innerHTML = data;
        })
        .catch(error => 
		{
            console.error('Error fetching pending leaves:', error);
            document.getElementById("rejectedRequestsContent").innerHTML = '<p style="color: red;">Failed to load data.</p>';
        });
}

function closerejectedLeaveRequests() 
{
    document.getElementById("rejectedRequestsModal").style.display = "none";
