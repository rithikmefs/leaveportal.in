...
?>
<br><br>
<button type="button" class="sub1" style="margin-left:1065px;" onclick="showPendingRequests()">PENDING LEAVE REQUESTS</button>
...
<div id="pendingRequestsModal" style="display: none; position: fixed; top: 20%; left: 19%; width: 55%; background-color: white; border: 1px solid #ccc; box-shadow: 0 4px 8px rgba(0,0,0,0.2); padding: 20px; z-index: 1000;border-radius:10px;">
    ...
    <span class="close-btn" onclick="closePendingRequests()">×</span>
</div>
...
<div style='background-color:#FFFFFF;border-radius:10px;width:50%;min-height:560px;max-height:750px;margin-left:-70;box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);'>
<br><br>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') 
{
    ...
    if ($confirmation) 
	{
		...
		foreach ($leaveDatesToCheck as $leaveDate) 
		{
			...
			$sqlCheckLeave = "SELECT * FROM leave WHERE empid = '$empid' AND leavedate = '$leaveDate' AND action = 'A'";
			$resCheckLeave = sqlsrv_query($conn, $sqlCheckLeave);

			$sqlCheckLeaveRequest = "SELECT * FROM leaverequest WHERE empid = '$empid' AND leavedate = '$leaveDate' and pending = 'Y'";
			$resCheckLeaveRequest = sqlsrv_query($conn, $sqlCheckLeaveRequest);

			$sqlCheckLeaveRejection = "SELECT * FROM leaverequest WHERE empid = '$empid' AND leavedate = '$leaveDate' and pending = 'N'";
			$resCheckLeaveRejection = sqlsrv_query($conn, $sqlCheckLeaveRejection);
			...
			if($resCheckLeaveRequest && sqlsrv_fetch_array($resCheckLeaveRequest, SQLSRV_FETCH_ASSOC))
			{
				$leaveExists2 = true;
				break;
			}
			if($resCheckLeaveRejection && sqlsrv_fetch_array($resCheckLeaveRejection, SQLSRV_FETCH_ASSOC))
			{
				$leaveExists3 = true;
				break;
			}			
		...
		elseif ($leaveExists3) 
		{
			echo '<b style="color: red;">The selected date for leave has already been rejected.</b>';
		}		
		elseif ($leaveExists2) 
		{
			echo '<b style="color: red;">The selected date for leave is currently pending.</b>';
		}		
            ...
            if ($selectedDays == 1) // 1DAY
			{
				...
				if($selectedLeaveType=='COMPO')
				{
					...
					{ 
						...
						$sql1 = "INSERT INTO leaverequest (empid, leavedate, compodate, reason, leavetype, time, pending)
								 VALUES ('$empid', '$selectedLeaveDate1', '$compodatesStr[0]', '$formReason', '$selectedLeaveType', '$currentDateTime', 'Y')";
			...
			elseif ($selectedDays == 2) //2 DAYS
			{
				...
				if($selectedLeaveType=='COMPO')
				{
					...
					{ 
						...
						$sql1 = "INSERT INTO leaverequest (empid, leavedate, compodate, reason, leavetype, time, pending)
								 VALUES ('$empid', '$selectedLeaveDate1', '$compodatesStr[0]', '$formReason', '$selectedLeaveType', '$currentDateTime', 'Y')";
						$sql2 = "INSERT INTO leaverequest (empid, leavedate, compodate, reason, leavetype, time, pending)
								 VALUES ('$empid', '$selectedLeaveDate2', '$compodatesStr[1]', '$formReason', '$selectedLeaveType', '$currentDateTime', 'Y')";
						$res1 = sqlsrv_query($conn, $sql1);
			...
			{             
				if($selectedLeaveType=='COMPO')
				{
					...
					{ 
						...
						$sql2 = "INSERT INTO leaverequest (empid, leavedate, compodate, reason, leavetype, time, pending)
								 VALUES ('$empid', '$selectedLeaveDate2', '$compodatesStr[1]', '$formReason', '$selectedLeaveType', '$currentDateTime', 'Y')";
						$sql3 = "INSERT INTO leaverequest (empid, leavedate, compodate, reason, leavetype, time, pending)
								 VALUES ('$empid', '$selectedLeaveDate3', '$compodatesStr[2]', '$formReason', '$selectedLeaveType', '$currentDateTime', 'Y')";
