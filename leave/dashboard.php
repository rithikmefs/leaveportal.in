...
function getLeavesTakenPerMonth($empid) 
{
	...
	FROM leave WHERE empid = '$empid' AND leavetype = 'CASUAL'  AND action = 'A' GROUP BY YEAR(leavedate), MONTH(leavedate);";
...
if(($empid=='999' && $power == 'Y') || ($empid=='1108' && $power == 'Y' ))//Admin Dashboard
{
	...
	$sql7 = "select count(*) as cnt from leave where empid='$empid' AND leavetype!='COMPO' AND action = 'A'";
	...
	if(!$holiday)
	{
		...
		$sql3 = "SELECT COUNT(empid) AS leaveemp FROM leave WHERE CAST(leavedate AS DATE) = CAST(GETDATE() AS DATE)
		         AND empid NOT IN ('1118', '1119') AND action = 'A';";
	...
	$sql4 = "SELECT e.name AS leaveempname,reason FROM leave l JOIN emp e ON l.empid = e.empid 
				 WHERE CAST(l.leavedate AS DATE) = CAST(GETDATE() AS DATE) AND l.empid NOT IN ('1118', '1119') AND action = 'A';";
...
$sql10 = "SELECT COUNT(DISTINCT e.empid) AS aspirantscount FROM emp e WHERE 
          ...
          WHERE MONTH(l.leavedate) = MONTH(GETDATE()) AND YEAR(l.leavedate) = YEAR(GETDATE())AND l.action = 'A');";	
...
$sql11 = "SELECT e.name AS aspirants,MAX(l.leavedate) AS lastleavedate FROM emp e LEFT JOIN leave l ON e.empid = l.empid
          WHERE l.action = 'A' AND e.flag = 'Y' AND e.emptype != 'TEST' AND e.empid NOT IN ('1108', '1070')AND e.empid NOT IN (
          SELECT DISTINCT empid FROM leave 
          WHERE action = 'A' AND MONTH(leavedate) = MONTH(GETDATE()) AND YEAR(leavedate) = YEAR(GETDATE()))
          GROUP BY e.name 
		  ORDER BY lastleavedate;";
...
while($row11 = sqlsrv_fetch_array( $res11, SQLSRV_FETCH_ASSOC))
{
	...
	{	
		...
		$totalaspirants1 .= '<b>'.$aspirants.'</b>[Last leave on '.$formattedLastleavedate.']&nbsp;&nbsp;<br>';	
		$k++;
...
Work Aspirants of the month<br>
<span style="font-size:18px;display:inline-block;width:19.5%;border-bottom:2px solid black;margin-top:10px;margin-bottom:10px;"></span><br>
<i><?php echo $totalaspirants; ?></i>
</p>
...
<div class="popup" id="popup">
  ...
  <div id="popup-content" style='font-family:poppins;'>Loading...</div>
    ...
    <table style="margin-left:425px;" id='employeelist'>
        <tr>
...
<center>
<div id='bottom'>
<img src="../leave/images/print.png" id="download" class="sub" style="width:1px;height:1px;" title="click to download the pdf">
</div>
</center>
...
document.addEventListener('DOMContentLoaded', function() 
{
    ...
    if (!storedBirthday)
	{
		...
		{		
			...
			var content2 = document.getElementById('bottom'); 
