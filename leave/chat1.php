    ...
    if (!isPaused) 
    {
        ...
        .then(response => 
        {
            ...
            data.forEach(msg => 
            {
				...
				// Function to get the latest occurrence of a given weekday (0 = Sunday, 1 = Monday, etc.)
				function getLatestWeekday(dayIndex) {
					const tempDate = new Date(currentDate);
					tempDate.setDate(currentDate.getDate() - ((currentDate.getDay() - dayIndex + 7) % 7));
					return tempDate.toDateString();
				}

				// Get the latest occurrences of weekdays
				const weekdays = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
				let dayLabel = msgDate.toDateString(); // Default label
				...
				if (isToday) {
					...
					// Check if msgDate matches the latest occurrence of any weekday
					for (let i = 0; i < 7; i++) {
						if (msgDate.toDateString() === getLatestWeekday(i)) {
							dayLabel = weekdays[i];
							break;
						}
					}
				...
				if (previousDate === null || msgDate.toDateString() !== previousDate.toDateString()) 
				{
					...
					//chatContainer.appendChild(dayDiv); // Append the day divider to the chat container
					...
					{
						dayDiv.style.marginLeft = '15px'; // Margin for dates
				...
				previousDate = msgDate; // Update the previous date
    ...
    if (!isPaused) 
    {
		...
		data.forEach(msg => 
		{
				...
				if (previousDate === null || msgDate.toDateString() !== previousDate.toDateString()) 
				{
					...
					{
						dayDiv.style.marginLeft = '155px'; // Margin for dates
						dayDiv.style.width = '140px'; // Width for a specific date
						dayDiv.style.textAlign = 'center';
						dayDiv.style.display = 'inline-block';						
					}
					else 
					{
						...
						dayDiv.style.overflowWrap = 'break-word'; 
						dayDiv.style.whiteSpace = 'pre-wrap'; 	
						dayDiv.style.marginLeft = '170px'; // Margin for dates
						dayDiv.style.width = '140px'; // Width for a specific date
						dayDiv.style.textAlign = 'center';
						dayDiv.style.display = 'inline-block';
					}
